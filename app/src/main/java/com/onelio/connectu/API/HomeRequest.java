...
public class HomeRequest {
  ...
  private static void putNotificationInJ(NotificationData data) throws JSONException {
    ...
    JSONObject tcontent =
        new JSONObject(
            content.toString()); // Prevent ConcurrentModificationException by adding a temp obj
    ...
    if (!app.notifications.has(data.getType())
        || !app.notifications
            .getJSONObject(data.getType())
            .toString()
            .contains(jstring.toString())) {
