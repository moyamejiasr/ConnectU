...
public class ProfesoresViewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Picasso.with(getBaseContext()).load(data.getPicture()).transform(new BlurTransform(ProfesoresViewActivity.this)).into(imgb);
        } else {
    ...
    public void onEmailClick(View view) {
        Intent emailIntent = new Intent(Intent.ACTION_SENDTO, Uri.fromParts("mailto", data.getEmail(), null));
        emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Mensaje de alumno de asigantura " + data.getSubject());
    ...
    public void onTutoriaClick(View view) {
        ...
        alert.setPositiveButton(getString(R.string.tutoria_create), new AlertManager.AlertCallBack() {
            @Override
        ...
        alert.setNegativeButton(getString(R.string.tutoria_cancel), new AlertManager.AlertCallBack() {
            @Override
            public void onClick(boolean isPositive) {
            }
        });
