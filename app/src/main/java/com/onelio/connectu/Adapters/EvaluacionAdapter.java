...
public class EvaluacionAdapter extends RecyclerView.Adapter<EvaluacionAdapter.ViewHolder> {
    ...
    public EvaluacionAdapter(Context context, OnItemClickListener listener, List<EvaluacionData> myDataset) {
        events = myDataset;
        ...
        if (data.getTypeID().equals("2")) {
            if (!data.isOpen()) {
                ...
                if (now.getTime() < data.getStart().getTime()) {
                    //Before
                    holder.icon.getDrawable().mutate().setColorFilter( Color.parseColor("#BDBDBD"), PorterDuff.Mode.SRC_IN );
                ...
                if (data.isCompleted()) {
                    //Opened and sended
                    holder.icon.getDrawable().mutate().setColorFilter( Color.parseColor("#00BFA5"), PorterDuff.Mode.SRC_IN );
                    holder.step.setText(context.getString(R.string.evaluacion_card_open_sended));
                } else {
                    //Opened but not sended
                    holder.icon.getDrawable().mutate().setColorFilter( Color.parseColor("#0091EA"), PorterDuff.Mode.SRC_IN );
