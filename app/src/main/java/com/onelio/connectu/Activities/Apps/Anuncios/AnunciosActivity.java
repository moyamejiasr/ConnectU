...
public class AnunciosActivity extends AppCompatActivity {
  ...
  private void markReadAll() {
    ...
    request.markAllRead(
        new AnunciosRequest.AnunciosCallback() {
          @Override
          public void onResult(final boolean onResult, final String message) {
            AnunciosActivity.this.runOnUiThread(
                new Runnable() {
                  @Override
                  public void run() {
                    if (onResult && message.equals("True")) {
                      Toast.makeText(
                              getBaseContext(),
                              getString(R.string.message_marked),
                              Toast.LENGTH_SHORT)
                          .show();
                      reloadAnuncios();
                    } else {
                    ...
                    if (onResult) {
                      ...
                      if (!error.handleError(message)) {
                        FirebaseCrash.report(
                            new Exception(
                                "Exception found in AnunciosActivity while reload: " + message));
  ...
  AnunciosAdapter.OnItemClickListener onClick =
      new AnunciosAdapter.OnItemClickListener() {
        @Override
        public void onItemClick(int item, View v) {
          try {
            ...
            alertManager.setPositiveButton(
                "Ok",
                new AlertManager.AlertCallBack() {
                  @Override
                  public void onClick(boolean isPositive) {}
                });
            alertManager.setMessage(
                getString(R.string.error_unexpected), getString(R.string.error_unable_display));
            alertManager.show();
          }
        }
      };
