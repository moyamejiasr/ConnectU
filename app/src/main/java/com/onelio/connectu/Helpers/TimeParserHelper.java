...
public class TimeParserHelper {
  ...
  public static String getDifference(Context context, Date startDate, Date endDate) {
    ...
    if (elapsedDays > 0) {
      result =
          result
              ...
              + " "
              + context.getString(R.string.view_horario_days)
              + " ";
    }
    ...
    if (elapsedHours > 0) {
      result =
          result
              + String.valueOf(elapsedHours)
              + " "
              + context.getString(R.string.view_horario_hours)
              + " ";
    }
    ...
    if (elapsedMinutes > 0) {
      result =
          result
              ...
              + " "
              + context.getString(R.string.view_horario_minutes)
              + " ";
    }
        ...
        if (cal.get(Calendar.MONTH) == current.get(Calendar.MONTH)) {
          if (cal.get(Calendar.DAY_OF_MONTH) == current.get(Calendar.DAY_OF_MONTH)) {
            if (cal.get(Calendar.HOUR_OF_DAY) == current.get(Calendar.HOUR_OF_DAY)) {
              ret =
                  context.getString(R.string.date_time_prev)
                      + " "
                      + context.getString(R.string.date_time_moment);
            } else {
              String result =
                  String.valueOf(current.get(Calendar.HOUR_OF_DAY) - cal.get(Calendar.HOUR_OF_DAY));
              ret =
                  context.getString(R.string.date_time_prev)
                      + " "
                      + result
                      + " "
                      + context.getString(R.string.date_time_hours);
            }
          } else {
            String result =
                String.valueOf(current.get(Calendar.DAY_OF_MONTH) - cal.get(Calendar.DAY_OF_MONTH));
            ret =
                context.getString(R.string.date_time_prev)
                    + " "
                    + result
                    + " "
                    + context.getString(R.string.date_time_days);
          }
        } else {
          ...
          ret =
              context.getString(R.string.date_time_prev)
                  ...
                  + result
                  + " "
                  + context.getString(R.string.date_time_months);
        }
      } else {
        ret = "";
      }
