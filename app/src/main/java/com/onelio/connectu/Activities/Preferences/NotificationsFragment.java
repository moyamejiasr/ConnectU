...
public class NotificationsFragment extends PreferenceFragmentCompat {
    ...
    int getIdByTime(int time) {
        ...
        for (int i = 0; i < list.length; i++) {
            if (list[i].equals(String.valueOf(time)))
                loc = i;
        }
        return loc;
    }
    ...
    CheckBoxPreference.OnPreferenceChangeListener onDisplayChange = new Preference.OnPreferenceChangeListener() {
        @Override
    ...
    ListPreference.OnPreferenceChangeListener onDelayChange = new Preference.OnPreferenceChangeListener() {
        @Override
        public boolean onPreferenceChange(Preference preference, Object newValue) {
    ...
    SwitchPreferenceCompat.OnPreferenceChangeListener onSetAlarm = new Preference.OnPreferenceChangeListener() {
        @Override
        public boolean onPreferenceChange(Preference preference, Object newValue) {
            NotificationManager manager = new NotificationManager(getContext());
            ListPreference delay = (ListPreference)findPreference("delayNotifications");
