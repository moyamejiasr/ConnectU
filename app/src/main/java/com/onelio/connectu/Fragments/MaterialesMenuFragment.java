...
public class MaterialesMenuFragment extends Fragment implements MaterialesActivity.OnBackPressed {
    ...
    private void loadNewLocation() {
        ...
        request.loadMaterialesLoc(url, new MaterialesRequest.MaterialsCallback() {
            @Override
    ...
    public boolean shouldLeave() {
        if (url.isEmpty() || isSessionEnded) {
            ...
            if (!isLoading) {
                ...
                backPressedTimes++;
                if (backPressedTimes == 4) //If user have tried 4 times when loading means that he really want to go out
                    return true;
            }
            return false;
        }
