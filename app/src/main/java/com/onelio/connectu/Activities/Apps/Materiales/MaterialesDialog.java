...
public class MaterialesDialog extends Dialog {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    lExit.setOnClickListener(
        new View.OnClickListener() {
          @Override
          public void onClick(View v) {
            if (!isDownloading) MaterialesDialog.super.onBackPressed();
          }
        });
  ...
  private View.OnClickListener onOpen =
      new View.OnClickListener() {
        @Override
        public void onClick(View v) {
          ...
          if (AppManager.isStoragePermissionGranted(activity)) {
            final File file =
                new File(
                    AppManager.getFileSaveLoc(Common.MATERIALES_TEMP_FILE),
                    Common.MATERIALES_TEMP_FILE + "." + data.getType());
                          ...
                          public void run() {
                            ...
                            if (onResult) {
                              ...
                              intent.setDataAndType(
                                  Uri.fromFile(file),
                                  AppManager.getMimeType(file.getParent() + "/" + file.getName()));
                              try {
                                ...
                                activity.runOnUiThread(
                                    new Runnable() {
                                      ...
                                      public void run() {
                                        ...
                                        alert.setMessage(
                                            getContext().getString(R.string.app_name_error),
                                            getContext().getString(R.string.error_find_open));
  ...
  private View.OnClickListener onDownload =
      new View.OnClickListener() {
        ...
        public void onClick(View v) {
          ...
          if (AppManager.isStoragePermissionGranted(activity)) {
            final File file =
                new File(AppManager.getFileSaveLoc(data.getSubjectName()), data.getFileName());
            progress.setVisibility(View.VISIBLE);
            request.downloadFile(
                data,
                file,
                        ...
                        new Runnable() {
                          ...
                          public void run() {
                            ...
                            if (onResult) {
                              Toast.makeText(
                                      getContext(),
                                      getContext().getString(R.string.materiales_saved_at)
                                          + data.getSubjectName(),
                                      Toast.LENGTH_LONG)
                                  .show();
                            } else {
                          ...
                          public void run() {
                            ...
                            if (onResult) {
                              ...
                              intent.putExtra(
                                  ...
                                  getContext().getString(R.string.materiales_sharing_you));
                              intent.putExtra(Intent.EXTRA_TEXT, data.getFileName());
                              try {
                                activity.startActivity(
                                    Intent.createChooser(
                                        intent,
                                        getContext().getString(R.string.materiales_share_button)));
                              } catch (ActivityNotFoundException ex) {
                                if (activity == null) return;
                                activity.runOnUiThread(
                                    new Runnable() {
