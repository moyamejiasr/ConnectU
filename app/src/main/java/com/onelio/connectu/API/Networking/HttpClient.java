...
public class HttpClient {
  ...
  private void cookieMaker(
      Context
          context) { // Method to prevent exception caused by cookieJar being null even if there is
    // no login after
    app.cookieJar =
  ...
  private void initClient(Context context, boolean big) {
    ...
    if (app.cookieJar == null) cookieMaker(context);
  ...
  public HttpClient(
      Context context,
      boolean big) { // Evaluacion case (Cause Evaluacion is reaaaaaaaaallllyyy sloooow)
  ...
  public Call get(String url, Callback callback) throws IOException {
    Request request =
        new Request.Builder().url(url).addHeader("X-Requested-With", "es.ua.uacloud").build();
