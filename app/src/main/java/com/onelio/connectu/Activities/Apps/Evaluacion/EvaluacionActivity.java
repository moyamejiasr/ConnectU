...
public class EvaluacionActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_year) {
            ...
            dialog.setOnUpdated(new EvaluacionFilterDialog.EvResponse() {
                @Override
    ...
    private void loginIn() {
        request.loginService(new EvaluacionRequest.EvaluacionCallback() {
            @Override
    ...
    private void updateYear(boolean isFirst, int year) {
        title.setText(getString(R.string.title_activity_evaluacion) + " (" + app.academicYears.get(year).getYear() + ")");
    ...
    private void doRequest() {
        ...
        request.fetchEvents(new EvaluacionRequest.EvaluacionCallback() {
            @Override
            public void onResult(final boolean onResult, final String message) {
                EvaluacionActivity.this.runOnUiThread(new Runnable() {
                    @Override
    ...
    EvaluacionAdapter.OnItemClickListener onClick = new EvaluacionAdapter.OnItemClickListener() {
        ...
        public void onItemClick(int item) {
            ...
            if (!data.getTypeID().equals("9")) {
                final ProgressDialog progress = ProgressDialog.show(EvaluacionActivity.this, getString(R.string.app_name), getString(R.string.evaluacion_loading), true);
                request.getFullDataFrom(data, new EvaluacionRequest.EvaluacionCallback() {
                    @Override
                    public void onResult(final boolean onResult, final String message) {
                        EvaluacionActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                ...
                                if (onResult) {
                                    ...
                                    EvaluacionViewDialog dialog = new EvaluacionViewDialog(EvaluacionActivity.this, data);
                                    dialog.show();
                                } else {
                                    ErrorManager error = new ErrorManager(getBaseContext());
                                    if (!error.handleError(message)) {
