...
public class UAUpdate extends IntentService {
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    isSilence =
        bundle.getString(Common.INTENT_KEY_UPDATE_TYPE).equals(Common.UAUPDATE_TYPE_SILENCE);
  ...
  AcademicYearRequest.UserDataCallback callback =
      new AcademicYearRequest.UserDataCallback() {
        @Override
            ...
            try {
              switch (loc) {
                ...
                case SUBJECTS:
                  ...
                  if (request.academicYears.get(yloc).getSubjectsData().size()
                      != 0) { // Has subjects?
                    // Continue searching
                    subject = request.academicYears.get(yloc).getSubjectsData().get(sloc).getId();
                    request.loadTeachersByYearAndSubject(
                        year, subject, false, callback); // Load teachers
                  } else {
  ...
  void updateHorario() {
    ...
    calendar.set(
        Calendar.DAY_OF_MONTH, Calendar.getInstance().getActualMinimum(Calendar.DAY_OF_MONTH));
    hstart = calendar.getTimeInMillis() / 1000;
    calendar.add(Calendar.MONTH, 1);
    calendar.set(
        Calendar.DAY_OF_MONTH, Calendar.getInstance().getActualMaximum(Calendar.DAY_OF_MONTH));
  ...
  ScheduleRequest.ScheduleCallback hcallback =
      new ScheduleRequest.ScheduleCallback() {
        @Override
        public void onCompleted(boolean onResult, String message) {
          if (onResult) {
            switch (loc) {
