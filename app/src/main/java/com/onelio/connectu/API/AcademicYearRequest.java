...
public class AcademicYearRequest {
  ...
  private static String DATES_URL =
      "https://cvnet.cpd.ua.es/uaTutorias/Emisor/HorariosPresenciales";
  private static String SUBJECTS_URL =
      "https://cvnet.cpd.ua.es/uaTutorias/Emisor/GetAsignaturasXaHorario";
  private static String TEACHERS_ALU_URL =
      "https://cvnet.cpd.ua.es/uaTutorias/Emisor/getHTAsignaturaAlumno";
  ...
  public void loadTeachersByYearAndSubject(
      final String year, final String sid, boolean all, final UserDataCallback callback) {
    String json = "{\"Cod\":" + sid + ",\"Curso\":\"" + year + "\"}";
    String url = TEACHERS_ALU_URL;
    if (all) url = TEACHERS_URL;
          ...
          public void onNavigationComplete(boolean isSuccessful, String body) {
            if (isSuccessful) {
              ...
              for (Element eteacher : selects) {
                ...
                if (description != null) teacher.setDescription(description.html());
  ...
  public int getSubjectLoc(int yearLoc, String id) {
    for (int i = 0; i < academicYears.get(yearLoc).getSubjectsData().size(); i++) {
      if (academicYears.get(yearLoc).getSubjectsData().get(i).getId().equals(id)) return i;
    }
    return 0;
  }
  ...
  public int getYearLoc(String year) {
    for (int i = 0; i < academicYears.size(); i++) {
      if (academicYears.get(i).getYear().equals(year)) return i;
    }
    return 0;
  }
