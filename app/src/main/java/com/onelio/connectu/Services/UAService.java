...
public class UAService extends Service {
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    if (!App.isFirstLaunch) {
      ...
      login.createSession(
          new LoginRequest.LoginCallback() {
            @Override
              ...
              if (onResult) {
                ...
                try {
                  if (resultUser.getBoolean("logged")) {
                    // Continue to login
                    login.loginAccount(
                        resultUser.getString("email"), resultUser.getString("pass"), onUserLogin);
                  }
                ...
                if (!message.equals(ErrorManager.FAILED_CONNECTION)) {
                  FirebaseCrash.report(
                      new Exception("Failed UAService login with error: " + message));
                }
              }
            }
          });
  ...
  LoginRequest.LoginCallback onUserLogin =
      new LoginRequest.LoginCallback() {
        @Override
        public void onLoginResult(final boolean onResult, final String message) {
          if (onResult) {
            ...
            // Updates
            FirebaseCrash.log(
                "Trying update in UAService with last time " + String.valueOf(app.lastUpdateTime));
            if (!message.equals("_ISFIRST")
                && UpdaterHelper.needsSilenceUpdate(app.lastUpdateTime)) {
              // Silence Update
  ...
  void userSearchForNews() {
    ...
    if (app.newNotifications.length() == 1) {
      ...
      // More than one alert
      title =
          getString(R.string.notifi_more_title_have)
              + " "
              + String.valueOf(app.newNotifications.length())
              + " "
              + getString(R.string.notifi_more_title_more);
    ...
    if (app.newNotifications.length() != 0) {
      ...
      if (display) {
        notif =
            new android.support.v7.app.NotificationCompat.Builder(getBaseContext())
                .setAutoCancel(true)
