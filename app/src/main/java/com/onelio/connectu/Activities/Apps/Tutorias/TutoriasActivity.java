...
public class TutoriasActivity extends AppCompatActivity {
    ...
    private void updateYear(boolean isFirst, int year) {
        title.setText(getString(R.string.title_activity_tutorias) + " (" + app.academicYears.get(year).getYear() + ")");
    ...
    private void doRequest() {
        request.loginService(new TutoriasRequest.TutoriasCallback() {
            @Override
            public void onResult(final boolean onResult, final String message) {
                TutoriasActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (onResult) {
