...
public class TutoriasViewActivity extends AppCompatActivity {
  ...
  private void
      initializeConnection() { // Used to initialize communication with the UACloud service that we
                               // are going to use
    if (id != null && !id.isEmpty()) { // Empty or null means new one
      request.fetchTutoriaById(
          id,
          new TutoriasRequest.TutoriasCallback() {
            @Override
            public void onResult(final boolean onResult, final String message) { // Get Data
              TutoriasViewActivity.this.runOnUiThread(
                  new Runnable() {
                    @Override
                    public void run() {
                      if (onResult) {
                        isLoading = false;
                        request.markTutoriaReadId(
                            id,
                            new TutoriasRequest.TutoriasCallback() {
                              @Override
