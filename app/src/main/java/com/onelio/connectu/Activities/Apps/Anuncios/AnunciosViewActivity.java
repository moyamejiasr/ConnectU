...
public class AnunciosViewActivity extends AppCompatActivity {
      ...
      request.loadAnuncio(
          ...
          new AnunciosRequest.AnunciosCallback() {
            ...
            public void onResult(final boolean onResult, final String message) {
              AnunciosViewActivity.this.runOnUiThread(
                  new Runnable() {
                    @Override
                    public void run() {
                      progress.hide();
  ...
  void setData() {
    ...
    if (Build.VERSION.SDK_INT >= 21) {
      getWindow()
          .setNavigationBarColor(Color.parseColor(ColorHelper.getColor(data.getType().charAt(0))));
      getWindow()
          .setStatusBarColor(Color.parseColor(ColorHelper.getColor(data.getType().charAt(0))));
    }
  ...
  AnunciosRequest.AnunciosCallback listener =
      new AnunciosRequest.AnunciosCallback() {
        @Override
        public void onResult(final boolean onResult, final String message) {
          AnunciosViewActivity.this.runOnUiThread(
